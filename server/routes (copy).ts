...
const JWT_SECRET: string = (() => {
  ...
  // ============================================================================
  // RATE LIMITING (Two-tier: IP backstop first, then per-user after auth)
  // ============================================================================
  ...
  const userTradeLimiter = rateLimit({
    windowMs: 60 * 1000,
    max: 30,
    ...
    legacyHeaders: false,
    skip: skipHealthCheck,
    keyGenerator: (req: any) => (req.userId ? `user:${req.userId}` : `ip:${ipKeyGenerator(req as any)}`),
  });
  ...
  const botLimiter = rateLimit({
    ...
    standardHeaders: true,
    legacyHeaders: false,
    keyGenerator: (req: any) => `bot:${req.headers["x-bot-secret"] || ipKeyGenerator(req as any)}`,
  });

  // ============================================================================
  // PRECISION-SAFE SOL PARSING (No floating-point math)
  // ============================================================================

  function parseSolToLamports(solAmount: string | number): bigint {
    const solStr = String(solAmount).trim();

    // Strict: only digits with optional single dot, no negatives, no exponent
    if (!/^[0-9]+(\.[0-9]+)?$/.test(solStr)) {
      throw new Error("Invalid SOL amount format");
    }

    const [wholeRaw, fracRaw = ""] = solStr.split(".");
    const whole = wholeRaw || "0";

    let frac = fracRaw;
    if (frac.length > 9) frac = frac.slice(0, 9);
    else frac = frac.padEnd(9, "0");

    return BigInt(whole + frac);
  }

  const MIN_TRADE_LAMPORTS = 1_000_000n; // 0.001 SOL
  const MAX_TRADE_LAMPORTS = 100_000_000_000n; // 100 SOL

  function validateTradeAmount(lamports: bigint): void {
    if (lamports <= 0n) throw new Error("Trade amount must be positive");
    if (lamports < MIN_TRADE_LAMPORTS) throw new Error("Trade amount too small (minimum 0.001 SOL)");
    if (lamports > MAX_TRADE_LAMPORTS) throw new Error("Trade amount too large (maximum 100 SOL)");
  }
...
// DexScreener price fetch with request coalescing + short TTL cache
...
const pendingDexRequests = new Map<string, Promise<PriceCache | null>>();
...
async function fetchDexScreenerPriceInternal(tokenAddress: string): Promise<PriceCache | null> {
  try {
    ...
    if (!solanaPair?.priceNative) return null;

    const priceLamports = Math.max(
      1,
      Math.floor(parseFloat(solanaPair.priceNative) * 1_000_000_000)
    );
    ...
    return { priceLamports, decimals, liquidityUsd, volume24hUsd, fetchedAt: Date.now() };
  } catch (e) {
...
async function fetchDexScreenerPrice(tokenAddress: string): Promise<PriceCache | null> {
  const cached = dexScreenerCache.get(tokenAddress);
  if (cached && (Date.now() - cached.fetchedAt) < PRICE_CACHE_TTL_MS) {
    return cached;
  }

  const pending = pendingDexRequests.get(tokenAddress);
  if (pending) return pending;

  const promise = fetchDexScreenerPriceInternal(tokenAddress);
  pendingDexRequests.set(tokenAddress, promise);

  try {
    const result = await promise;
    if (result) dexScreenerCache.set(tokenAddress, result);
    return result;
  } finally {
    pendingDexRequests.delete(tokenAddress);
  }
}
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Health check (used by uptime monitors and rate-limit skip)
  app.get("/api/health", (req, res) => {
    res.json({ ok: true, ts: Date.now() });
  });

  // IP backstop rate limit for all API routes
