...
class HeliusService {
  ...
  async getTokenAnalysis(mintAddress: string) {
    try {
      ...
      const tokenInfo = (metadata as any)?.[0];
      ...
      // Security analysis
      const securityFlags = [];
      const supplyValue = (supply as any)?.value?.uiAmount || 0;
  ...
  async getTransactionDetails(signature: string) {
    try {
      ...
      return {
        signature,
        transaction: (parsed as any)?.[0] || null,
        timestamp: Date.now(),
      };
    } catch (error) {
  ...
  async search(query: string) {
    ...
    try {
      ...
      const owner = (accountInfo as any)?.value?.owner;
