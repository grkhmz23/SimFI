...
// Config
const SOL_PRICE_CACHE_TTL = 30_000;        // 30 seconds - fresh cache
const SOL_PRICE_STALE_TTL = 5 * 60_000;    // 5 minutes - stale but usable
...
export async function getSolPrice(): Promise<number | null> {
  ...
  if (cachedSolPrice && (now - cachedSolPrice.timestamp) < SOL_PRICE_STALE_TTL) {
...
export function getCachedSolPrice(): number | null {
  ...
  const now = Date.now();
  if ((now - cachedSolPrice.timestamp) < SOL_PRICE_STALE_TTL) {
    return cachedSolPrice.price;
  }
...
export function getSolPriceCacheStatus(): {
  ...
  return {
    ...
    available: ageMs < SOL_PRICE_STALE_TTL,
    source: cachedSolPrice.source,
    ageMs,
    isFresh: ageMs < SOL_PRICE_CACHE_TTL,
    isStale: ageMs >= SOL_PRICE_CACHE_TTL,
  };
}
